...
class FXPanel:
    ...
    # WHICH price feed served the bars (see real_data_feed.PRICE_SOURCE_LABELS).
    # The live dashboard shows this: a panel built from the curated export looks
    # identical to a live one otherwise, and mislabelling it as live is the
    # difference between a current forecast and a months-old one.
...
def _assemble_panel(ohlc: pd.DataFrame, macro: pd.DataFrame, news: pd.DataFrame,
                    source: str, price_source: str = "unknown") -> FXPanel:
    tech = compute_technical_features(ohlc)
    # LAZY scorer: build_sentiment_features only runs FinBERT on the RAW ('text')
    # path; the PRE-SCORED ('daily_score') path -- which every cached-archive and
    # live-dashboard build uses -- never touches it. Constructing it eagerly cost
    # ~30s per process (a subprocess segfault-probe that imports transformers,
    # then a second in-process import), all of it wasted on the pre-scored path.
    scorer = None if "daily_score" in news.columns else _get_scorer()
...
def build_fx_panel(
    ...
    if source == "real":
        ...
        if real is not None:
            ...
            # Name the feed that actually served the bars. The old wording
            # ("MT5 live / yfinance fallback") described the chain rather than
            # the outcome, so a build served entirely from the stale curated
            # export still logged as if it were live.
            try:
                from data.real_data_feed import PRICE_SOURCE_LABELS
            except ImportError:
                # A log label must never abort a panel build (see the
                # stale-module note in dashboard/app.py). Falls back to the raw
                # tag, which is still readable.
                PRICE_SOURCE_LABELS = {}
            _psrc = real.get("price_source", "unknown")
            print(f"[data] Using LIVE data: {len(ohlc)} candles from {real.get('interval','?')} "
                  f"price feed via {PRICE_SOURCE_LABELS.get(_psrc, _psrc)} "
                  f"(latest bar {ohlc.index[-1]}), "
                  f"{real['n_raw_headlines']} raw headlines (GDELT + RSS), macro: {macro_src}.")
            panel = _assemble_panel(ohlc, macro, news, source="real", price_source=_psrc)
            if export_artifacts:
