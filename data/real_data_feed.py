...
def fetch_all_news(feed_urls=None, gdelt_days: int = 60, gdelt_window_days: int = 5,
    ...
    if os.environ.get("FX_SKIP_GDELT") != "1":
        ...
        # SKIP it when the caller already asked for a short window (an
        # incremental top-up on a fresh archive): the main pass above already
        # covers those days at >= this resolution, so the extra 7 windows were
        # ~45s of pure rate-limit sleeping for headlines we just fetched.
        if gdelt_days > 14:
    ...
    # rate limits.
    # Google News + the RSS feeds live on DIFFERENT hosts and share no rate
    # limit, so fetch them concurrently instead of serially (they were ~4 x 10s
    # RSS timeouts plus the Google lanes, all I/O-bound waiting). GDELT above
    # stays strictly serial -- its rate limit is per-client and unforgiving.
    from concurrent.futures import ThreadPoolExecutor
    jobs = {"google-news": lambda: fetch_google_news(days=7, lanes=cfg.gnews_lanes)}
    for _u in feed_urls:
        jobs[_u] = (lambda u=_u: fetch_fxstreet_feed(u))
    with ThreadPoolExecutor(max_workers=min(6, len(jobs))) as _ex:
        futures = {name: _ex.submit(fn) for name, fn in jobs.items()}
        for name, fut in futures.items():
            try:
                f = fut.result()
            except Exception as e:                      # one bad feed must not sink the rest
                print(f"[real_data_feed] feed '{name}' failed (non-fatal): {e}")
                continue
...
def try_fetch_real_panel(ticker_symbol: str = "GC=F", interval: str = "5m", count: int = 1000) -> Optional[dict]:
    ...
    #   auto -- freshest available source (the dashboard's live path).
    ...
    _mt5_ok = cfg is not None and os.environ.get("FOREX_NO_MT5") != "1"
    if _mt5_ok:
        from data.mt5_feed import load_mt5_live, load_mt5_ohlc
        if src == "csv":
            ohlc = load_mt5_ohlc(cfg.name, interval)
            price_source = "mt5_csv" if ohlc is not None else None
        else:
            # "live" and "auto" both start at the attached terminal.
            ohlc = load_mt5_live(cfg.name, interval, count=count)
            price_source = "mt5_live" if ohlc is not None else None
            # In "auto" the curated CSV is deliberately NOT tried here. It is a
            # HISTORICAL source that ends at the last training export, so
            # reaching for it before yfinance meant any host without a
            # MetaTrader terminal -- Streamlit Cloud runs Linux, where the
            # MetaTrader5 package cannot even be installed -- silently served
            # months-old bars from a page labelled "live". yfinance is tried
            # next (below) and does serve current bars everywhere; the CSV
            # stays as the last resort after that.
    ...
    if ohlc is None or ohlc.empty:
        ...
        price_source = "yfinance" if ohlc is not None and not ohlc.empty else None
    if (ohlc is None or ohlc.empty) and src == "auto" and _mt5_ok:
        # Both live feeds are unreachable. Serve the curated export so the page
        # still renders, but price_source now says so and the caller labels it.
        from data.mt5_feed import load_mt5_ohlc
        ohlc = load_mt5_ohlc(cfg.name, interval)
        price_source = "mt5_csv_stale" if ohlc is not None else None
    if ohlc is None or ohlc.empty:
        return None

    # ONE TIMEZONE CONVENTION: tz-naive UTC. MetaTrader returns tz-naive broker
    # time and the curated CSV is tz-naive, but yfinance returns tz-AWARE
    # exchange time. Mixing them raises "Cannot subtract tz-naive and tz-aware"
    # the moment anything compares a panel stamp against a probe or a news
    # timestamp -- which is every freshness check and the recent-headlines
    # lookup. Normalise here, at the one point every price source funnels
    # through, rather than defensively at each comparison.
    ...
    if last_date is not None:
        ...
        # LIVE-INFERENCE CAP. GDELT is rate-limited (~6s between windows), so the
        # top-up cost scales with the gap: a 9-day gap measured ~80s, which
        # dominated the whole live prediction. A live forecast only reads the last
        # `lookback` bars, so news older than that window cannot affect it --
        # backfilling a week of history to serve a 60-bar window is wasted wall
        # clock. FOREX_MAX_TOPUP_DAYS (set by the dashboard's live path) caps it;
        # unset = full gap, so build_news_archive.py / training builds still do
        # the complete backfill.
        _cap = int(os.environ.get("FOREX_MAX_TOPUP_DAYS", "0") or 0)
        if _cap > 0 and gap_days > _cap:
            print(f"[real_data_feed] live top-up capped at {_cap}d (archive gap is "
                  f"{gap_days}d); run build_news_archive.py for the full backfill.")
            gap_days = _cap
    ...
    return {
        ...
        "interval": interval,
        # Which feed actually served the bars, so a caller can label the panel
        # honestly instead of assuming the first source in the chain won.
        "price_source": price_source or "unknown",
    }


# Human-readable names for the price_source tags above.
PRICE_SOURCE_LABELS = {
    "mt5_live": "attached MetaTrader 5 terminal (live)",
    "yfinance": "Yahoo Finance (live)",
    "mt5_csv": "curated MetaTrader 5 export (historical)",
    "mt5_csv_stale": "curated MetaTrader 5 export — LIVE FEEDS UNREACHABLE",
    "unknown": "unknown",
}


def probe_latest_bar(ticker_symbol: str, interval: str = "1h"):
    """Cheapest possible 'what is the latest bar?' check, for freshness banners
    and cache keys. Tries the attached terminal first, then Yahoo over a short
    period -- fetch_gold_candles asks for 730d of hourly bars, which is far too
    much to download just to read one timestamp.

    Returns (source_tag, pd.Timestamp) or (None, None). Never raises.
    """
    from data.pairs import pair_for_ticker
    cfg = pair_for_ticker(ticker_symbol)
    if cfg is not None and os.environ.get("FOREX_NO_MT5") != "1":
        try:
            from data.mt5_feed import load_mt5_live
            df = load_mt5_live(cfg.name, interval, count=1)
            if df is not None and len(df):
                return "mt5_live", pd.Timestamp(df.index[-1])
        except Exception:
            pass
    try:
        import yfinance as yf
        period = "5d" if interval.endswith(("m", "h")) else "1mo"
        h = yf.Ticker(ticker_symbol).history(period=period, interval=interval)
        if h is not None and len(h):
            ts = pd.Timestamp(h.index[-1])
            # tz_convert(None) -> UTC then drop the tz, matching the panel's
            # convention. tz_localize(None) would keep the local wall clock and
            # silently misreport the bar's age by the exchange's UTC offset.
            return "yfinance", (ts.tz_convert(None) if ts.tzinfo else ts)
    except Exception:
        pass
    return None, None
