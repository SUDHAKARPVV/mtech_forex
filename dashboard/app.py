...
# ---- stale-module self-heal (must run before anything imports from data.*) ----
# Streamlit re-executes app.py from disk on every rerun, but sys.modules pins
# imported modules at whatever the server process booted with. On Streamlit
# Cloud a git push hot-reloads app.py WITHOUT restarting the process, so a NEW
# app.py runs against a data.* package from BEFORE the push -- and any symbol
# added in that same commit raises ImportError on a line that is perfectly
# valid on disk. (_build_live_panel already carried a narrower version of this
# guard for build_fx_panel's signature; this one covers the whole family.)
#
# Keep the required-symbol lists below in step with what this file imports from
# data.*, so a future commit that adds a symbol heals itself instead of taking
# the deployed app down until someone reboots it by hand.
_REQUIRED_DATA_SYMBOLS = {
    "data.real_data_feed": ("PRICE_SOURCE_LABELS", "probe_latest_bar",
                            "news_archive_path", "try_fetch_real_panel"),
    "data.dataset": ("build_fx_panel", "time_split", "FXPanel"),
    "data.pairs": ("PAIRS", "get_pair", "panel_csv_path"),
}


def _heal_stale_data_modules() -> bool:
    """Reload the data.* family if any module in sys.modules predates a symbol
    this file needs. Returns True if a reload happened."""
    import importlib
    stale = [m for m, names in _REQUIRED_DATA_SYMBOLS.items()
             if (mod := sys.modules.get(m)) is not None
             and any(not hasattr(mod, n) for n in names)]
    if not stale:
        return False
    print(f"[app] stale data.* modules detected ({', '.join(stale)}) -- reloading in-place ...")
    # Dependency order, leaves first, so each module's own from-imports rebind
    # against the reloaded leaves rather than the old objects.
    for name in ("data.pairs", "data.mt5_feed", "data.technical_indicators",
                 "data.sentiment", "data.real_data_feed", "data.dataset"):
        if name in sys.modules:
            try:
                importlib.reload(sys.modules[name])
            except Exception as e:      # never let the heal itself kill the page
                print(f"[app] reload of {name} failed (continuing): {type(e).__name__}: {e}")
    return True


_HEALED = _heal_stale_data_modules()
...
if _HEALED:
    # Anything cached was built BY the modules just reloaded -- e.g. an FXPanel
    # produced by the previous class object is not an instance of the new one,
    # so attribute access on it would fail in confusing ways. Drop the caches;
    # this costs one rebuild, and only on the first run after a hot-reload.
...
def load_panel_and_splits(pair: str = "XAU/USD"):
    ...
    panel = build_fx_panel(pair=pair, n_days=10000, seed=9,
                           source="panel", real_interval="1h",   # H1 project (panel source ignores it)
...
# ---- live-path stage timing (so "why is this slow?" is answerable in the UI) ----
_stage_times: dict = {}
_INSTRUMENTED = {"done": False}


def _instrument_stages():
    """Wrap the live-build stages once, accumulating wall time per stage into
    _stage_times. Profiling showed the cost is ~entirely news (GDELT fetch +
    re-scoring new headlines), NOT the price/macro feeds or model inference --
    so surface that breakdown instead of leaving users to guess."""
    ...
    def wrap(mod, name, label):
        ...
        def g(*a, **k):
            ...
            try:
                ...
                _stage_times[label] = _stage_times.get(label, 0.0) + (_t.time() - t0)
        setattr(mod, name, g)
    try:
        import data.real_data_feed as _rdf
        import data.mt5_feed as _m5
        wrap(_rdf, "fetch_gdelt_news", "News: GDELT fetch (rate-limited)")
        wrap(_rdf, "fetch_google_news", "News: Google News RSS")
        wrap(_rdf, "fetch_fxstreet_feed", "News: other RSS feeds")
        wrap(_rdf, "_grow_news_archive", "News: score + merge into archive")
        wrap(_rdf, "fetch_real_macro", "Macro fetch (Yahoo + BLS)")
        wrap(_rdf, "align_scored_news_to_bars", "Align news → bars")
        wrap(_m5, "load_mt5_live", "Price: MT5 live pull")
        wrap(_m5, "load_mt5_ohlc", "Price: CSV fallback")
        _INSTRUMENTED["done"] = True
    except Exception:
        pass


def _raw_ohlc_tail(pair: str, interval: str, n: int):
    """Last n RAW bars (open/high/low/close/volume) exactly as received -- the
    input to feature engineering, shown so the transformation chain is
    inspectable rather than a black box. Tries the attached terminal, then
    Yahoo, so the lineage view is populated on hosts without MetaTrader (e.g.
    Streamlit Cloud). Returns [] only if both feeds are unreachable."""
    df = None
    try:
        from data.mt5_feed import load_mt5_live
        df = load_mt5_live(pair, interval, count=n)
    except Exception:
        df = None
    if df is None or not len(df):
        try:
            from data.pairs import get_pair
            from data.real_data_feed import fetch_gold_candles
            df = fetch_gold_candles(ticker_symbol=get_pair(pair).ticker,
                                    interval=interval, count=n)
        except Exception:
            df = None
    if df is None or not len(df):
        return []
    d = df.tail(n).reset_index()
...
def _latest_bar_key(pair: str, interval: str) -> str:
    ...
    On probe FAILURE this must NOT return a constant: the panel build then falls
    back to the curated CSV export (which ends at the last training export), and
    a constant key would pin that stale panel in cache_resource for the whole
    server lifetime -- so the page would keep serving training-era data even
    after the terminal came back. Bucketing by the hour makes it retry instead.
    """
    fs = _live_feed_status(pair, interval)
    if fs["live_bar"] is not None:
        return f"{fs['source']}@{fs['live_bar']}"
    return "no-live-feed@" + pd.Timestamp.now("UTC").strftime("%Y-%m-%dT%H")


@st.cache_data(ttl=120, show_spinner=False)
def _live_feed_status(pair: str, interval: str) -> dict:
    """Which feed is serving fresh bars, and what is its latest bar?

    Probes the attached terminal FIRST, then Yahoo. Probing only MetaTrader was
    the bug behind "not fetching live rate": on any host without a terminal --
    Streamlit Cloud runs Linux, where the MetaTrader5 package cannot be
    installed at all -- this always reported "not live" and the page fell back
    to the curated export, which ends at the last training build.

    Cached briefly because the page calls it more than once per render.
    """
    out = {"live_bar": None, "source": None, "error": None}
    try:
        from data.pairs import get_pair
        from data.real_data_feed import probe_latest_bar
        src, bar = probe_latest_bar(get_pair(pair).ticker, interval)
        if bar is not None:
            out["live_bar"], out["source"] = pd.Timestamp(bar), src
        else:
            out["error"] = ("no live feed reachable — MetaTrader terminal not attached "
                            "and Yahoo Finance returned no bars")
    except Exception as e:
        out["error"] = f"{type(e).__name__}: {e}"
    return out


def _news_archive_latest(pair: str):
    """Timestamp of the newest headline in this pair's scored archive, or None."""
    try:
        from data.pairs import get_pair
        from data.real_data_feed import news_archive_path
        p = news_archive_path(get_pair(pair).ticker)
        if not os.path.exists(p):
            return None
        ts = pd.to_datetime(pd.read_csv(p, usecols=["timestamp"])["timestamp"],
                            errors="coerce", utc=True).dropna()
        return ts.max().tz_localize(None) if len(ts) else None
    except Exception as e:
        print(f"[live] news archive probe failed (non-fatal): {e}")
        return None


def _news_archive_covers_window(pair: str) -> bool:
    """Does the cached archive reach far enough forward to give the model's
    input window any headlines at all? The window is the last `lookback` bars,
    so an archive older than that leaves every sentiment feature dead."""
    latest = _news_archive_latest(pair)
    if latest is None:
        return False
    window_start = (pd.Timestamp.now("UTC").tz_convert(None)
                    - pd.Timedelta(hours=DATA_CFG.lookback))
...
def _build_live_panel(pair: str, interval: str, bar_key: str, fetch_news: bool):
    ...
    if fetch_news:
        ...
        # A live forecast only reads the last `lookback` bars, so topping up more
        # news history than that window cannot change the prediction -- and GDELT
        # is rate-limited (~6s/window), so an unbounded gap dominated the runtime
        # (measured 80s of a 133s build). Cap to the window (+1d slack).
        _os.environ["FOREX_MAX_TOPUP_DAYS"] = str(max(2, DATA_CFG.lookback // 24 + 1))
        # Skip GDELT on the LIVE path. It is the slow, rate-limited leg (measured
        # 80s of a 133s build; frequently HTTP 429 then retry-backoff), and its
        # value is HISTORICAL depth -- irrelevant to a 60-bar window. Google News
        # RSS + the direct feeds return the freshest headlines in seconds and are
        # not rate-limited. Full-history backfill still uses GDELT via
        # build_news_archive.py; unset this to restore it here.
        _os.environ.setdefault("FX_SKIP_GDELT", "1")
    elif _news_archive_covers_window(pair):
        _os.environ["FOREX_OFFLINE_NEWS"] = "1"          # cached archive (fast)
        _os.environ.pop("FOREX_MAX_TOPUP_DAYS", None)
    else:
        # The cached archive ends BEFORE the model's input window starts, so
        # going offline would feed the model 60 bars with no headlines at all --
        # every sentiment feature dead, which is a silent train/serve mismatch,
        # not merely "slightly stale news". Top up just far enough to cover the
        # window, RSS-only (no GDELT), which costs seconds rather than minutes.
        _os.environ.pop("FOREX_OFFLINE_NEWS", None)
        _os.environ["FOREX_MAX_TOPUP_DAYS"] = str(max(2, DATA_CFG.lookback // 24 + 2))
        _os.environ["FX_SKIP_GDELT"] = "1"
        print("[live] news archive does not reach the input window — "
              "running a bounded RSS-only top-up instead of the offline path.")
...
def compute_live_forecast(hybrid, xgb, pair="XAU/USD", fetch_news=False):
    ...
    # Match the interval the checkpoint was TRAINED on -- feeding a window of the
    # wrong bar size would be a train/serve mismatch. The project is fixed on H1,
    # so that is the fallback when a checkpoint predates the meta field.
    _cmeta = load_json(os.path.join(checkpoint_dir(pair), "meta.json")) or {}
    _interval = _cmeta.get("interval", "1h")
    ...
    _t_start = _time.time()
    _t_probe = _time.time()
    _bar_key = _latest_bar_key(pair, _interval)
    _probe_s = _time.time() - _t_probe
    _t_panel = _time.time()
    fresh = _build_live_panel(pair, _interval, _bar_key, fetch_news)
    _panel_s = _time.time() - _t_panel
    _stages = dict(_stage_times)          # empty on a cache hit (nothing rebuilt)
    _cache_hit = not _stages
    ...
    return {"forecast": fc, "band": band, "deep_forecast": deep, "xgb_trust": xtrust,
            ...
            # full-precision stamp of the panel's final bar: the date-only
            # last_date above cannot be compared against the live probe's bar
            # (it reads as up to 23h of phantom drift).
            "last_bar_ts": str(fresh.dates[-1]),
            # Which feed actually served these bars, and how current the news
            # archive behind the sentiment features is. Both are shown in the
            # freshness banner: without them a panel built from the curated
            # export is indistinguishable from a live one.
            "price_source": getattr(fresh, "price_source", "unknown"),
            "news_latest": (str(_nl) if (_nl := _news_archive_latest(pair)) is not None else None),
            "timing": {"total_s": _time.time() - _t_start, "panel_s": _panel_s,
                       "bar_probe_s": _probe_s, "stages": _stages,
                       "cache_hit": _cache_hit, "fetch_news": bool(fetch_news)},
            "n_bars": int(len(fresh.close)),
            "xq": xq.numpy(), "xt": xt.numpy(), "rc": rc.numpy(), "xgp": xgp.numpy(),
            "feature_names": fn, "nq": int(nq),
            # genuinely RAW broker bars (pre-engineering) for the data-lineage view
            "raw_ohlc": _raw_ohlc_tail(pair, _interval, L),
            "window_ts": [str(d) for d in fresh.dates[-L:]],
...
# ----------------------------- layer detail popups -----------------------------
LAYER_DETAILS = {
    # Widths are read from config so they can never drift from the model again
    # (this block previously claimed 18/12, a stale count from before the drift
    # and envelope features took technical 12 -> 18, i.e. quant 18 -> 24).
    "Quant input": ("Quantitative input",
        f"(B, {DATA_CFG.lookback}, {DATA_CFG.n_technical_features + DATA_CFG.n_macro_features})",
        f"The technical + macroeconomic stream: {DATA_CFG.lookback} trailing hourly (H1) bars, "
        f"{DATA_CFG.n_technical_features + DATA_CFG.n_macro_features} features each.",
        [f"{DATA_CFG.n_technical_features} technical features — OHLC log-returns, RSI, MACD-hist, Bollinger "
         "width, volume-z, ATR%, ROC, %K, EMA ratio, envelope deviation, Bollinger %B, and the drift/"
         "drift-t-stat conditional-mean state",
         f"{DATA_CFG.n_macro_features} macro features — short-rate z, 10y-yield change, dollar-index return, "
         "CPI yoy, CPI mom, days-since-CPI",
...
if page.startswith("🏠"):
    ...
    # Data-driven status line: reads the committed per-pair metrics so it can
    # never drift from the actual results again.
    _st_bits = []
    for _s in ("XAUUSD", "XAGUSD", "EURUSD"):
        _m = load_json(f"results/pair_metrics/{_s}.json")
        if _m and "hybrid" in _m:
            _st_bits.append(f"{_s} {_m['hybrid']['DirAcc']:.3f}")
    _st_line = " · ".join(_st_bits) if _st_bits else "n/a"
    st.info(
        f"**Honest status — what this project found.**\n\n"
        f"**1. Direction is not predictable at H1.** Test-set directional accuracy sits at ~0.50 for every pair "
        f"({_st_line}), seed-stable to ±0.002. GARCH edges ahead only via its momentum drift, and neither model "
        f"beats the *always-up base rate* — the control this project applies throughout. Roughly fifteen framings "
        f"(smoothed targets, selective/trend-gated trading, threshold calibration, order-flow, Fair-Value-Gap, "
        f"CFTC positioning) were tested and all landed at the base rate. This is a rigorous efficient-market "
        f"result, not a tuning gap.\n\n"
        f"**2. Volatility/magnitude *is* forecastable** — the productive axis. The model forecasts move-magnitude "
        f"on par with ATR% and GARCH-σ (point estimates favour it, but a Hansen Model Confidence Set finds all "
        f"three statistically indistinguishable — see Results).\n\n"
        f"**3. The genuine contribution is calibrated uncertainty.** Adaptive Conformal Inference turns the "
        f"model's mis-calibrated Gaussian band (73% actual coverage at a 90% target) into intervals that hold "
        f"~90% coverage under regime shift — a distribution-free guarantee GARCH's parametric bands lack. "
        f"It is live on the Prediction page.\n\n"
        f"News coverage is no longer a limitation (98–99.9% of test bars carry headlines), and a modality "
        f"ablation showed news and macro add ~0 to magnitude skill over price/volatility features.", icon="ℹ️")
...
# ================== 2. ARCHITECTURE & LAYER I/O ==================
    ...
    digraph G {
      ...
      qin  [label="Quant input\\n(B,60,QW)" fillcolor="#94A3B8" fontcolor="#0F172A"];
    ...
    # keep the diagram's quant width tied to config (was a hardcoded, stale 18)
    dot = dot.replace("(B,60,QW)",
                      f"(B,{DATA_CFG.lookback},"
                      f"{DATA_CFG.n_technical_features + DATA_CFG.n_macro_features})")
...
# ===================== 3. DATA & FEATURES =====================
    ...
    if not os.path.exists(_ppath):
        ...
        st.caption("Yahoo rates/dollar-index + BLS CPI, transformed to stationary form and forward-filled onto "
                   "the hourly (H1) bar grid — each daily release is shifted one day before it reaches a bar, "
                   "so no bar sees a figure published after it closed. Shown over the recent window for "
                   "readability.")
...
# ===================== 4. LIVE PREDICTION =====================
    ...
    # [:16] not [:10]: on an H1 panel consecutive slider positions are one
    # HOUR apart, so a date-only caption repeats for ~24 origins in a row and
    # the slider looks stuck.
    dates = [str(panel.dates[t])[:16] for t in origins]
    ...
    st.caption(f"Origin bar: **{dates[idx]}**  ·  test window {idx+1} of {n}")
    ...
    fig.update_layout(height=320, template="plotly_white", margin=dict(l=0, r=0, t=10, b=0),
                      xaxis_title="forecast horizon (bars ahead, H1)", yaxis_title="cumulative log-return")
    st.plotly_chart(fig, use_container_width=True)
    ...
    if f"live_fc_{PAIR}" in st.session_state:
        ...
        nd = F.get("news_days")
        # Show the bar's full stamp: on an H1 panel a date alone cannot say
        # WHICH of the day's 24 bars this forecast starts from.
        _lb = str(F.get("last_bar_ts") or F["last_date"])[:16]
        st.success(f"Live forecast from the latest bar **{_lb}** "
                   f"(close ${F['last_close']:,.2f}) · {F['n_bars']:,} bars fetched"
                   # The window is `lookback` HOURLY bars, not days -- the old
                   # wording is left over from the daily-bar pipeline.
                   + (f" · **{nd}/{DATA_CFG.lookback}** bars in the window carry live news"
                      if nd is not None else ""))
        # ---- WHERE THE TIME WENT ----
        _tm = F.get("timing") or {}
        if _tm:
            _tot = _tm.get("total_s", 0.0)
            with st.expander(f"⏱️ Where the {_tot:.0f}s went — per-stage breakdown"
                             + (" (cached: nothing rebuilt)" if _tm.get("cache_hit") else "")):
                _stg = _tm.get("stages") or {}
                if _stg:
                    _rows = sorted(_stg.items(), key=lambda kv: -kv[1])
                    _acc = sum(v for _, v in _rows)
                    _df_t = pd.DataFrame(
                        [{"stage": k, "seconds": round(v, 1),
                          "% of run": round(v / _tot * 100, 1) if _tot else 0.0} for k, v in _rows]
                        + [{"stage": "feature engineering + model inference (remainder)",
                            "seconds": round(max(0.0, _tot - _acc - _tm.get("bar_probe_s", 0)), 1),
                            "% of run": round(max(0.0, _tot - _acc) / _tot * 100, 1) if _tot else 0.0}])
                    st.dataframe(_df_t, use_container_width=True, hide_index=True)
                    st.caption(
                        "Measured, not estimated. The price pull, macro fetch and the model's own "
                        "forward pass are milliseconds-to-seconds; when this run is slow it is the **news** "
                        "path — GDELT is rate-limited (~6s between query windows) and any genuinely new "
                        "headlines must be FinBERT-scored before they can be aligned to bars. With "
                        "'fresh news' unticked the cached archive is used and the whole build is ~10s.")
                else:
                    st.caption(
                        f"Panel served from cache ({_tm.get('panel_s', 0):.1f}s) — no fetching or feature "
                        f"rebuild was needed, because no new bar has closed since the last run. The bar "
                        f"probe took {_tm.get('bar_probe_s', 0):.1f}s.")
        # ---- DATA-FRESHNESS BANNER ----
        # The panel build falls through a chain of feeds and the curated CSV
        # export -- which ends at the last training export -- is the last link.
        # That fallback used to be invisible, so a months-old forecast looked
        # identical to a live one. Report the feed the PANEL actually used
        # (F["price_source"]), not the first feed in the chain.
        try:
            from data.real_data_feed import PRICE_SOURCE_LABELS
        except ImportError:
            # Belt and braces behind _heal_stale_data_modules(): a label lookup
            # must never be what takes the forecast page down. .get() below
            # falls through to the raw tag ("mt5_live"), which is still legible.
            PRICE_SOURCE_LABELS = {}
        _cur_interval = (meta or {}).get("interval", "1h")   # the checkpoint's trained bar size
        _fs = _live_feed_status(PAIR, _cur_interval)
        _panel_last = pd.Timestamp(F.get("last_bar_ts") or F["last_date"])
        _live_bar = _fs.get("live_bar")
        _psrc = F.get("price_source", "unknown")
        _plabel = PRICE_SOURCE_LABELS.get(_psrc, _psrc)
        if _psrc in ("mt5_csv", "mt5_csv_stale", "unknown"):
            st.error(
                f"**Not live — showing the curated export.** No live feed was reachable "
                f"({_fs.get('error') or 'both the MetaTrader terminal and Yahoo Finance failed'}), "
                f"so the panel was built from the curated export, which ends "
                f"**{_panel_last:%Y-%m-%d %H:%M}** — the last training export, not current market "
                f"data. Check this machine's internet access, or start and log in to the MT5 "
                f"terminal, then click predict again; this stale state is no longer cached "
                f"indefinitely.", icon="🔌")
        elif _live_bar is not None and abs((_live_bar - _panel_last).total_seconds()) > 3600:
            st.warning(
                f"**Stale panel.** The {_plabel} feed's latest bar is "
                f"**{_live_bar:%Y-%m-%d %H:%M}** but this forecast was built from a panel ending "
                f"**{_panel_last:%Y-%m-%d %H:%M}**. Click predict again to rebuild on the current "
                f"bar.", icon="⚠️")
        else:
            # Bar stamps are FEED server time (MT5 brokers commonly run UTC+2/+3),
            # so a current bar can be stamped AHEAD of UTC and the raw difference
            # goes negative -- which reads as a bug. Clamp at zero and describe
            # anything within a bar of now as current; the number is only ever
            # meant to catch a days-old feed, not to be minute-accurate.
            _hrs = (pd.Timestamp.now("UTC").tz_convert(None) - _panel_last).total_seconds() / 3600.0
            _age = "current" if _hrs < 1 else f"~{_hrs:.0f}h old"
            st.caption(
                f"🟢 **Live feed OK** — source: {_plabel} · latest closed bar "
                f"{_panel_last:%Y-%m-%d %H:%M} ({_age}, feed server time). FX/metals close "
                f"Friday night and reopen Sunday evening, so an older bar across a weekend is expected, "
                f"not stale.")
        # ---- NEWS FRESHNESS ----
        # Prices and news come from independent feeds, so the panel can be live
        # while the sentiment features are dead. Report the news age separately;
        # news_days counts how many of the window's bars carry any headline.
        _nl = F.get("news_latest")
        if _nl:
            _nl_ts = pd.Timestamp(_nl)
            _nage = max(0.0, (pd.Timestamp.now("UTC").tz_convert(None) - _nl_ts).total_seconds() / 3600.0)
            _nd = F.get("news_days")
            if _nd == 0:
                st.warning(
                    f"**No news in the input window.** The scored archive ends "
                    f"**{_nl_ts:%Y-%m-%d %H:%M}** (~{_nage/24:.1f} days old), so none of the "
                    f"{DATA_CFG.lookback} bars the model reads carry a headline and every sentiment "
                    f"feature is at its neutral default. The price and macro streams are unaffected. "
                    f"Tick **fresh news** above, or run `scripts/build_news_archive.py`, to restore "
                    f"the sentiment tower.", icon="📰")
            else:
                st.caption(
                    f"📰 News archive latest **{_nl_ts:%Y-%m-%d %H:%M}** (~{_nage:.0f}h old) · "
                    f"{_nd}/{DATA_CFG.lookback} bars in the window carry at least one headline.")
        ...
        if _conf is None and bd is not None:
            st.caption(
                f"The green band is the model's **raw Gaussian σ** (≈68% nominal). Note it is "
                f"*mis-calibrated* — on gold it covers only ~73% at a 90% target. The calibrated "
                f"**Adaptive-Conformal** band has been fitted for gold so far; run "
                f"`python analysis/conformal_intervals_gold.py` for this pair to enable it here "
                f"(it needs `results/conformal_live_state_{PCFG.slug}.json`).")
        if _conf is not None:
            ...
            st.caption(
                ...
                + (f" ⚠️ {_capn}/10 horizons are at the calibration-scale cap (regime stress)."
                   if _capn else ""))
        _pm_live = load_json(f"results/pair_metrics/{PCFG.slug}.json") or {}
        _da_live = (_pm_live.get("hybrid") or {}).get("DirAcc")
        st.caption(
            "Price and macro are fetched **live**; news uses the up-to-date **cached archive** by default "
            "(tick the box to also pull fresh headlines live — slower, GDELT is rate-limited). The archive now "
            "covers ~98–99.9% of bars, so the cached path is not a coverage compromise. "
            + (f"**Direction on live data tracks the honest test-set figure ({_da_live:.3f}) — i.e. a coin flip; "
               f"treat the arrow as illustrative and the calibrated band above as the usable output.**"
               if _da_live is not None else ""))

        # ---------- 📥 INPUT DATA USED ----------
        st.divider()
        st.subheader("📥 Input data used for this prediction (last 60 bars)")
        # ---- DATA LINEAGE: raw bars -> engineered features -> normalised tensors ----
        _ln_ts = F.get("window_ts") or F.get("window_dates") or []
        _ln_names = F.get("feature_names") or []
        _ln_raw = F.get("window_raw")
        _ln_nq = int(F.get("nq") or 0)
        with st.expander("🔬 Data lineage — exactly how the raw feed becomes model input "
                         "(raw bars → engineered features → normalised tensors)"):
            st.markdown(
                f"The model never sees prices. Each forecast consumes a "
                f"**{DATA_CFG.lookback} × {DATA_CFG.n_total_features}** window, split into a quantitative "
                f"tensor **(1, {DATA_CFG.lookback}, {_ln_nq})** and a sentiment tensor "
                f"**(1, {DATA_CFG.lookback}, {DATA_CFG.n_total_features - _ln_nq})**. The three stages below "
                f"are the actual arrays from *this* prediction.")

            st.markdown("**Stage 1 — RAW market bars** (straight from the broker feed, no processing)")
            _ro = F.get("raw_ohlc") or []
            if _ro:
                _rdf2 = pd.DataFrame(_ro)
                for _c in _rdf2.columns:
                    if _rdf2[_c].dtype.kind == "f":
                        _rdf2[_c] = _rdf2[_c].round(3)
                st.dataframe(_rdf2.iloc[::-1].head(12), use_container_width=True, hide_index=True)
                st.caption(f"{len(_ro)} bars pulled; newest first. These OHLCV values are the only price "
                           f"input — everything below is derived from them (plus macro and news).")
            else:
                st.info("Raw bars unavailable (terminal not serving) — the panel used the CSV fallback.")

            st.markdown(f"**Stage 2 — ENGINEERED features** ({DATA_CFG.n_total_features}, raw scale, "
                        f"before normalisation)")
            if _ln_raw is not None and _ln_names:
                _wr2 = np.asarray(_ln_raw)
                _eng = pd.DataFrame(_wr2, columns=_ln_names)
                _eng.insert(0, "bar", _ln_ts[:len(_eng)])
                _tech_n = DATA_CFG.n_technical_features
                _macro_n = DATA_CFG.n_macro_features
                _tabs = st.tabs([f"🟦 Technical ({_tech_n})", f"🟩 Macro ({_macro_n})",
                                 f"🟪 Sentiment ({DATA_CFG.n_sentiment_features})"])
                _groups = [_ln_names[:_tech_n], _ln_names[_tech_n:_tech_n + _macro_n],
                           _ln_names[_tech_n + _macro_n:]]
                for _tab, _cols in zip(_tabs, _groups):
                    with _tab:
                        _sub = _eng[["bar"] + [c for c in _cols if c in _eng.columns]]
                        st.dataframe(_sub.round(5).iloc[::-1].head(12),
                                     use_container_width=True, hide_index=True)
                st.caption("Stationary transforms (log-returns, z-scores, ratios) — never raw price levels, "
                           "which are non-stationary (the ADF test on the Data page confirms it).")

            st.markdown("**Stage 3 — NORMALISED tensors** (what the network actually receives)")
            _xq, _xt = F.get("xq"), F.get("xt")
            if _xq is not None:
                _xq2, _xt2 = np.asarray(_xq), np.asarray(_xt)
                st.code(f"x_quant  {tuple(_xq2.shape)}   x_text {tuple(_xt2.shape)}\n"
                        f"last bar, quant[:8] = {np.round(_xq2[0, -1, :8], 3).tolist()}\n"
                        f"last bar, text[:8]  = {np.round(_xt2[0, -1, :8], 3).tolist()}", language="text")
                st.caption("Normalised with the TRAIN-split mean/std only (never test/live statistics) — "
                           "so a live bar is transformed exactly as training bars were, with no look-ahead. "
                           "Values are ~z-scores: 0 ≈ the training average, ±1 ≈ one training SD.")
        # Bar TIMESTAMPS, not dates. At H1 the 60-bar window spans ~2.5 days, so
        # a date-only label collapses ~24 rows onto one value: the FX-rate and
        # macro tables below could not be read bar by bar, and the charts drew
        # ~3 x-positions instead of 60. (Stage 2 of the lineage expander already
        # used window_ts, which is why it showed times and these did not.)
        # window_dates is the fallback for a forecast cached by an older build.
        wds = [str(t)[:16] for t in (F.get("window_ts") or F.get("window_dates") or [])]
        wcl, wraw = F.get("window_close"), F.get("window_raw")
        if wds and wcl is not None:
            ...
            with st.expander(f"📈 Live FX rates — last {DATA_CFG.lookback} bars (table, newest first)"):
                st.dataframe(pd.DataFrame({"bar (feed time)": wds,
                                           "close": np.round(wcl, 2)}).iloc[::-1],
                             use_container_width=True, hide_index=True)
            ...
            if wraw is not None and _macro_cols:
                with st.expander(f"📉 Macro feed used — {len(_macro_cols)} indicators over the window"):
                    ...
                    mdf.insert(0, "bar (feed time)", wds)
                    mfig = go.Figure()
            ...
            st.caption(f"These are exactly the rows the model consumed: {DATA_CFG.lookback} hourly bars of live "
                       "price → technical indicators, live macro, and live news sentiment, normalised with the "
                       "training statistics.")
    ...
    if not pm or "hybrid" not in pm:
        ...
        if mag and mag.get("aggregate"):
            ...
            if sig is not None:
                ...
                # THREE states, not two. The bootstrap tests the RANK metrics this
                # panel reports (spearman, large-move accuracy); DM/MCS test
                # SQUARED-ERROR loss of the scaled |move| forecast. Those can
                # legitimately disagree -- squared error is driven by a few extreme
                # moves and by scale calibration, while rank/classification skill is
                # robust to both. Silver is significant on every bootstrap
                # comparison (p<=0.006) yet MCS keeps all three; collapsing that to
                # "not significant" would erase a real difference from gold, which
                # is significant on none.
                _bps = [v.get("p") for k, v in ba.items() if isinstance(v, dict) and v.get("p") is not None]
                _boot_sig = bool(_bps) and max(_bps) < 0.05
                _mcs_drops = 0 < len(incl) < 3
                _dm = sig.get("dm", {})
                _dma = (_dm.get("model_vs_atr") or {}).get("p")
                _dmg = (_dm.get("model_vs_garch") or {}).get("p")
                if _boot_sig and _mcs_drops:
                    st.success(
                        f"**Statistically significant.** Every block-bootstrap comparison clears p<0.05 "
                        f"(vs ATR% p≈{pa:.3f}, vs GARCH-σ p≈{pg:.3f}) *and* the Model Confidence Set drops "
                        f"a baseline. A genuine edge, not noise.", icon="✅")
                elif _boot_sig:
                    st.info(
                        f"**Mixed evidence — significant on rank skill, not on squared error.**\n\n"
                        f"*Block bootstrap* (on the two metrics this panel reports): the Hybrid beats "
                        f"**both** baselines on rank skill **and** large-move accuracy, with every "
                        f"confidence interval excluding zero — vs ATR% p≈{pa:.3f}, vs GARCH-σ p≈{pg:.3f}. "
                        f"That is a real, well-powered result.\n\n"
                        f"*Diebold–Mariano / Hansen MCS* (on squared-error loss of the scaled magnitude "
                        f"forecast): not significant"
                        + (f" (DM p≈{_dma:.2f} / {_dmg:.2f})" if _dma is not None else "")
                        + f"; the MCS keeps all {len(incl)} forecasters.\n\n"
                        f"**These test different things and both are reported.** Squared error is dominated "
                        f"by a handful of extreme moves and by how well the forecast is *scaled*; rank and "
                        f"classification skill are robust to both. So the honest claim is: the Hybrid orders "
                        f"and classifies future move-size better than ATR% and GARCH-σ, without being "
                        f"demonstrably better in squared-error terms.", icon="🔬")
                else:
                    st.warning(
                        ...
                        f"advantage: vs ATR% p≈{pa:.3f}, vs GARCH-σ p≈{pg:.3f} (both >0.05); Diebold–Mariano "
                        f"ns. The autocorrelation of overlapping 10-bar windows shrinks the effective "
                        f"sample, so the small rank edge is within noise. **Honest conclusion: the Hybrid "
            ...
            st.caption(f"Scope: target is |net displacement| over 10 bars, not RMS realised vol. "
                       f"Significance = block-bootstrap + Diebold–Mariano + Hansen Model Confidence Set "
                       f"on the seed-9 forecasts (results/significance_magnitude_{PCFG.slug}.json)."
                       + ("" if sig is not None else
                          " Significance testing has not been run for this pair yet, so only the "
                          "point estimates above are shown."))
        else:
            # No magnitude run for this pair yet -- say so explicitly rather than
            # silently omitting the section (which reads as a broken page).
            st.divider()
            st.subheader("📐 Move-magnitude forecasting — model vs ATR% vs GARCH-σ")
            st.info(
                f"**Not computed for {PCFG.label} yet.** Direction is unpredictable at H1 for every pair, so "
                f"the productive question is *how big* the next move is. That experiment (3 seeds, scored "
                f"against the ATR% indicator and the classical GARCH-σ volatility model) has so far been run "
                f"for gold. It runs for this pair with:\n\n"
                f"`python scripts/run_multi_seed.py --seeds 9 36 99 --pair {PAIR} --interval 1h "
                f"--target magnitude`\n\n"
                f"The panel fills in automatically once `results/multi_seed_magnitude_{PCFG.slug}.json` "
                f"exists — nothing else to change.", icon="📐")
